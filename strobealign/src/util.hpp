...
/* long long version */
// static uint64_t msbll(uint64_t x) {
//     assert(x > 0);                 // if x is 0, the result is undefined
//     return 63 - __builtin_clz(x);  // count leading zeros (clz)
// }
...
template <typename T>
void save_vector(std::ostream& out, std::vector<T> const& vec) {
...
template <typename T>
void save_vector_fp(FILE* fp, std::vector<T> const& vec) {
    uint64_t vec_size = vec.size();
    fwrite(&vec_size, sizeof(vec_size), 1, fp);
    fwrite(&vec[0], sizeof(T), vec.size(), fp);
}

template <typename T>
void load_vector(std::istream& in, std::vector<T>& vec) {
    uint64_t vec_size = 0;
...
template <typename T>
void load_vector_fp(FILE* fp, std::vector<T>& vec) {
    uint64_t vec_size = 0;
    uint64_t val = fread(&vec_size, sizeof(vec_size), 1, fp);
    vec.resize(vec_size);
    val = fread(&vec[0], sizeof(T), vec_size, fp);
    if(!val){
        throw std::invalid_argument("Util.hpp: file loading failed\n");
    }
}
