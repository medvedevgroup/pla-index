...
struct bit_vector_builder {
    bit_vector_builder(uint64_t size = 0, bool init = 0) : m_size(size) {
        m_bits.resize(essentials_arank::words_for(size), uint64_t(-init));
        if (size) {
    ...
    void reserve(uint64_t num_bits) {
        m_bits.reserve(essentials_arank::words_for(num_bits));
    }
    ...
    inline void zero_extend(uint64_t n) {
        m_size += n;
        uint64_t needed = essentials_arank::words_for(m_size) - m_bits.size();
        if (needed) {
    ...
    void append(bit_vector_builder const& rhs) {
        ...
        m_bits.resize(essentials_arank::words_for(m_size));
...
struct bit_vector {
    ...
    uint64_t bytes() const {
        return sizeof(m_size) + essentials_arank::vec_bytes(m_bits);
    }
