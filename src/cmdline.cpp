...
CommandLineOptions parse_command_line_arguments_build(int argc, char **argv) {
    args::ArgumentParser parser("pla-index" );
    parser.helpParams.showTerminator = false;
    parser.helpParams.helpindent = 20;
    parser.helpParams.width = 90;
    parser.helpParams.programName = "pla-index";
    parser.helpParams.shortSeparator = " ";

    args::HelpFlag help(parser, "help", "Print help and exit", {'h', "help"});

    // build_index
    args::ValueFlag<std::string>genome_fasta(parser, "STRING", "Fasta file with one entry and only ACGT characters", {'g', "genome_fasta"},args::Options::Required);
    args::ValueFlag<std::string>suffix_array(parser, "STRING", "Suffix array of the genome in a binary file", {'s', "suffix_array"},args::Options::Required);
    ...
    args::Flag enable_fast_rank(parser, "r", "Whether to build bit vector to support fast rank query. [no]", {'r'});
...
CommandLineOptions parse_command_line_arguments_query(int argc, char **argv) {
    ...
    args::ValueFlag<std::string>genome_fasta(parser, "STRING", "Fasta file with one entry and only ACGT characters", {'g', "genome_fasta"},args::Options::Required);
    args::ValueFlag<std::string>suffix_array(parser, "STRING", "Suffix array of the genome in a binary file", {'s', "suffix_array"},args::Options::Required);
    args::ValueFlag<std::string>index_name(parser, "STRING", "File name where to save the index [genome_fasta.index]", {'i', "index"},args::Options::Required);
    args::ValueFlag<std::string>query_file(parser, "STRING", "Name of the file containing all the queries (one kmer per line)", {'q', "query"},args::Options::Required);
